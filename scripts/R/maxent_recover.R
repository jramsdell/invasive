...
calcMaxent <- function(newX, pi_base, pi_tilde){
  ...
  converged <- FALSE
  original_log_loss <-
    calcWeightExp(pi_tilde, -log(calcQL(newX, lambdas)))
  original_QL <- calcQL(newX, lambdas)
  step = 0.1
...
if(ALREADY_FOUND == FALSE){
  ...
  saveRDS(X, "./data/dataWithCells.rds")
}else{
  X <<- readRDS("./data/dataWithCells.rds")
}
